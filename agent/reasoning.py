...
def is_weather_only_mixed_plan(plan_config: Dict) -> bool:
    allowed_tools = set(plan_config.get("allowed_tools", []) or [])
    return plan_config.get("domain_scope") == "mixed" and allowed_tools == {"weather"}


def extract_city_by_rule(query: str) -> str:
    city_pattern = r"北京|上海|广州|深圳|杭州|成都|重庆|南京|武汉|西安|郑州|济南|青岛|天津|苏州|长沙|合肥|福州|厦门|昆明|南昌|宁波|无锡|佛山|东莞"
...
def build_mixed_weather_answer(query: str, tool_results: List[Dict], answer_scope_hint: str = "") -> str:
    weather_line = ""
    for tool_result in tool_results or []:
        if not isinstance(tool_result, dict):
            continue
        for item in tool_result.get("data", []) or []:
            ...
            if isinstance(content, dict):
                ...
                weather_line = content
                payload = None
            else:
                payload = None

            if payload and payload.get("data"):
                inner_items = payload.get("data") or []
                if inner_items and isinstance(inner_items[0], dict):
                    weather_line = inner_items[0].get("content", "")
            if weather_line:
                break
        if weather_line:
            break
    ...
    city = extract_city_by_rule(query)
    if city == "未知":
        return "我目前只能回答天气相关部分，但问题中没有明确城市，无法准确查询天气；餐饮推荐不在当前系统的回答范围内。"

    fallback = f"我目前只能回答天气相关部分。已识别城市为{city}，但天气查询暂时失败；餐饮推荐不在当前系统的回答范围内。"
    if answer_scope_hint:
        return f"{fallback}\n\n{answer_scope_hint}"
    return fallback
...
def optimize_answer(
    ...
    if answer_scope_hint:
        extra_requirements.append(answer_scope_hint)
...
""".format(
    ...
    try:
        ...
        return raw_answer.strip() if raw_answer else (context[0].strip() if context else "暂无可用答案")
...
def extract_city_for_weather(query: str) -> str:
    ...
    从查询中提取城市名称（用于天气工具）
    """
    city_name = extract_city_by_rule(query)
    if city_name != "未知":
        return city_name
...
def execute_rag_node(state: ReasoningState) -> dict:
    ...
    if is_weather_only_mixed_plan(plan_config):
        rag_result = {
            "success": True,
            "retrieval_results": [],
            "final_answer": "该问题为混合范围问题，仅处理天气相关部分。",
            "retrieve_rounds": 0,
            "error": None,
        }
        return {
            ...
            "step": "rag_completed",
            "messages": [AIMessage(content="Mixed weather-only query: skip RAG and go directly to weather tool.")],
        }
...
def judge_tool_node(state: ReasoningState) -> dict:
    ...
    if is_weather_only_mixed_plan(plan_config) and any(key in query for key in ["天气", "气温", "降雨", "寒潮", "高温", "气候"]):
        return {
            "tool_decision": ["weather"],
            "step": "judge_completed",
            "messages": [AIMessage(content="Mixed weather-only query: call weather tool directly")],
        }
    ...
    if plan_config.get("force_tool_call"):
        ...
        allowed_tools = set(plan_config.get("allowed_tools", []))
        if allowed_tools:
            suggested_tools = [tool_name for tool_name in suggested_tools if tool_name in allowed_tools]
    ...
    if plan_config.get("domain_scope") == "out_of_scope":
        return {
            "tool_decision": [],
            "step": "judge_completed",
            "messages": [AIMessage(content="Out-of-scope query: skip all tools.")],
        }

    tool_prompt = f"""
允许的工具范围：{plan_config.get("allowed_tools", []) or "ALL"}
回答范围提示：{plan_config.get("answer_scope_hint", "") or "无"}
你需要分析以下查询和现有RAG检索结果，判断是否需要调用外部工具，规则如下：
1. 仅当RAG结果无法满足查询需求，例如缺少实时或特定地区数据时，才调用工具。
2. 可调用工具列表：
{[f"{name}: {desc['description']}" for name, desc in TOOL_DEFINITIONS.items()]}
3. 输出格式仅返回工具名称列表，例如["weather"]或[]，不要输出额外解释。
4. 如果问题只能回答部分内容，必须严格限制在允许的工具范围内，不要为了完整性越界扩展。
...
"""
    ...
    allowed_tools = set(plan_config.get("allowed_tools", []))
    if allowed_tools:
        tool_list = [tool_name for tool_name in tool_list if tool_name in allowed_tools]
        if not tool_list and allowed_tools == {"weather"} and any(key in query for key in ["天气", "气温", "降雨", "寒潮", "高温", "气候"]):
            tool_list = ["weather"]
...
def optimize_answer_node(state: ReasoningState) -> dict:
    ...
    if not rag_result:
        ...
        if is_weather_only_mixed_plan(plan_config):
            optimized = build_mixed_weather_answer(
                query=query,
                tool_results=state.get("tool_results", []),
                answer_scope_hint=plan_config.get("answer_scope_hint", ""),
            )
        else:
